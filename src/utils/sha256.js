...
export class Sha256 {
  ...
  update(data) {
    if (this.finished) {
      throw new Error("SHA-256 digest already finished.");
    }
    ...
    while (position < bytes.length) {
      ...
      const input = bytes.subarray(position, position + space);
  ...
  digest() {
    if (!this.finished) {
      ...
      this.buffer[56] = hi >>> 24;
      this.buffer[57] = hi >>> 16;
      this.buffer[58] = hi >>> 8;
      this.buffer[59] = hi;
      this.buffer[60] = lo >>> 24;
      this.buffer[61] = lo >>> 16;
      this.buffer[62] = lo >>> 8;
      this.buffer[63] = lo;
      this.transform(this.buffer);
      this.finished = true;
    ...
    return this.hash.map(toHex).join("");
  }
}

export const sha256Bytes = (bytes) => new Sha256().update(bytes).digest();
