...
export default function BackupSettingsScreen({ navigation, route, onOpenMenu }) {
  ...
  const backupStats = useMemo(() => {
    ...
    return {
      photo: queuedStats.photo.count ? queuedStats.photo : scannedStats.photo,
      video: queuedStats.video.count ? queuedStats.video : scannedStats.video,
      file: queuedStats.file.count ? queuedStats.file : scannedStats.file,
      queue: queueStats,
    ...
    <View style={styles.container}>
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <MediaToggle
          icon="image"
          title="Photos"
          subtitle={`${backupStats.photo.count.toLocaleString()} items - ${bytesToGb(backupStats.photo.bytes)}`}
          value={photos}
        ...
        <MediaToggle
          icon="videocam"
          title="Videos"
          subtitle={`${backupStats.video.count.toLocaleString()} items - ${bytesToGb(backupStats.video.bytes)}`}
          value={videos}
        ...
        <MediaToggle
          icon="document-text"
          title="Files"
          subtitle={`${backupStats.file.count.toLocaleString()} items - ${bytesToGb(backupStats.file.bytes)}`}
          value={files}
        ...
        >
          <Ionicons
            ...
            size={16}
            color="#FFFFFF"
            style={isScanning ? styles.spinIcon : undefined}
          />
...
const MediaToggle = ({ icon, title, subtitle, value, onValueChange }) => (
